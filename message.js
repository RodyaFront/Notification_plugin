...
class MessageItemContstructor {
    ...
    resumeHide() {
        console.log('IS ENDLESS : ', this.$endless)
        if(this.$endless) {
            return
        }
...
export default class Message {
    ...
    createMessage({positions, title, text, type, duration}) {
        const message = new MessageItemContstructor({
            types: this.$types,
        })
    ...
    generateMessageOptions(options) {
        ...
        const duration = options && options.duration || options && options.duration === 0
            ? options.duration
